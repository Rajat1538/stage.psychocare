        ...
        <div class="card card-horizontal card-default card-md mb-4">
            ...
            <div class="card-body py-md-30">
                    ...
                    <ul class="atbd-breadcrumb nav">
                        ...
                        <li class="atbd-breadcrumb__item">
                            <a href="{{route('admin.manufacturingplant.list')}}">
                                {{ trans('label.manufacturing_plant_list') }}
                            </a>
                            <span class="breadcrumb__seperator">
                                <span class="la la-angle-right"></span>
                            </span>
                        ...
                        <li class="atbd-breadcrumb__item">
                        @if(isset($blogObjs) && !empty($blogObjs)) Manufacturing Plants Edit @else Manufacturing Plants Add @endif
                        </li>
                    </ul>
                </div>
                ...
                <div class="horizontal-form">
                    <!-- Form -->
                        @if(!empty($blogObjs))
							{!! Form::open(array('route' => ('admin.manufacturingplant.updatelist'),'method'=>'POST', 'files'=>true,'class' => 'form-horizontal','id' => 'form','enctype' => 'multipart/form-data')) !!}
							{!! Form::hidden("id",isset($blogObjs->id) ? $blogObjs->id : '', ['class' => 'form-control','id' => 'hiddenId']) !!}
						@else
							{{ Form::open(array('route' => 'admin.manufacturingplant.store', 'method' => 'POST', 'files'=>true,'class' => 'form-horizontal','id' => 'form','enctype' => 'multipart/form-data')) }}
                            ...
                            <div class="col-sm-9">
                                <div class="account-profile mb-4">
                                    ...
                                    <div class="ap-img pro_img_wrapper">
                                        {!! Form::file("image", array('id'=>'image', 'accept' => 'image/*', 'class' => 'invisible' ,'style' => 'position: absolute;', 'onchange' => 'PreviewImage(this)')) !!}

                                        <label for="image">
                                            <?php
                                                $mainImageUrl = URL::asset('resources/admin-asset/img/default.jpg');
                                                if(isset($blogObjs->image) && !empty($blogObjs->image)){
                                                    $mainImageUrl = asset('storage/app/public/uploads/ManufacturingPlant').'/'.$blogObjs->image;
                                                }
                                            ?>
                                            <input type="hidden" name="old_image" id="old_image" value="{{ isset($blogObjs->image) ? $blogObjs->image : '' }}">
                                    ...
                                    <div class="account-profile__title">
                                        ...
                                        @endif
                                        <span class="text-danger ml-20" id="errorImage"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group row mb-25">
                            ...
                            <div class="col-sm-9">
                                {!! Form::textarea('title', isset($blogObjs->title) ? $blogObjs->title : '', ['placeholder' => 'Enter Title', 'id' => 'title', 'class' => 'form-control add-another-rest', 'data-validation' => 'required']) !!}
                                @if($errors->has('title'))
                            ...
                            <div class="form-group row mb-25">
                                ...
                                <div class="col-sm-9 d-flex align-items-center">
                                    <div class="custom-control custom-switch switch-primary switch-md ">
                                        <input type="checkbox" name="status" class="custom-control-input" id="switch_status" onChange="changeManufacturingStatus(event.target);" {{ (isset($blogObjs->status) && $blogObjs->status == 1) || !isset($blogObjs->status) ? 'checked' : '' }}>
                        ...
                        <div class="form-group row mb-0">
                            ...
                            <div class="col-sm-9">
                                <div class="layout-button mt-25">
                                    <button type="submit" class="btn btn-sm btn-primary btn-add px-30" id="manufacturing_plant_validation">@if(isset($blogObjs) && !empty($blogObjs)) {{ trans('label.update') }} @else {{ trans('label.save') }} @endif</button>
                                    <a class="btn btn-default btn-squared border-normal bg-normal px-30 " href="{{ route('admin.manufacturingplant.list') }}">{{ trans('label.cancel') }}</a>
                                </div>
                            </div>
                        </div>
                    {!! Form::close() !!}
...
form.submit();
 ...
 $(input).parent().parent().siblings('#' + input.name + '_error').html('')
    ...
    // status change
	function changeManufacturingStatus(_this) {
