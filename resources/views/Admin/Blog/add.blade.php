                ...
                <div class="card-body py-md-30">
                    ...
                    <div class="horizontal-form">
                        ...
                        @if (!empty($Blog))
                            ...
                            {!! Form::hidden('id', isset($Blog->id) ? $Blog->id : '', [
                                'class' => 'form-control',
                                'id' => 'hiddenId',
                            ]) !!}
                        @else
                            {{ Form::open(['route' => 'admin.blogs.store', 'method' => 'POST', 'files' => true, 'class' => 'form-horizontal', 'id' => 'form', 'enctype' => 'multipart/form-data']) }}
                                    ...
                                    <div class="ap-img pro_img_wrapper">
                                        ...
                                        <label for="image">
                                            ...
                                            if (isset($Blog->image) && !empty($Blog->image)) {
                                                $mainImageUrl = asset('storage/app/public/uploads/blogs/image') . '/' . $Blog->image;
                                            }
                                            ?>
                                            <input type="hidden" name="old_image" id="old_image"
                                                value="{{ isset($Blog->image) && !empty($Blog->image) ? $Blog->image : '' }}">
                        ...
                        <div class="form-group row pb-2">
                            ...
                            <div class="col-sm-9">
                                {!! Form::textarea('title', isset($Blog->title) ? $Blog->title : '', [
                        ...
                        <div class="form-group row pb-2">
                            ...
                            <div class="col-sm-9">
                                {!! Form::textarea('description', isset($Blog->description) ? $Blog->description : '', [
                            ...
                            <div class="col-sm-9 d-flex align-items-center">
                                <div class="custom-control custom-switch switch-primary switch-md ">
                                    <input type="checkbox" name="status" class="custom-control-input" id="switch_status"
                                        ...
                                        {{ (isset($Blog->status) && $Blog->status == 1) || !isset($Blog->status) ? 'checked' : '' }}>
                        ...
                        <div class="form-group row pb-2">
                            <div class="col-sm-3 d-flex justify-content-end">
                                <label for="news" class="col-form-label color-dark fs-14 fw-500 align-center">
                                    Is This News?</span>
                                </label>
                            </div>
                            <div class="col-sm-9 d-flex align-items-center">
                                <div class="custom-control custom-switch switch-primary switch-md ">
                                    <input type="checkbox" name="is_this_news" class="custom-control-input" id="switch_news"
                                        onChange="changeBlogNews(event.target);"
                                        {{ (isset($Blog->is_this_news) && $Blog->is_this_news == 1) || !isset($Blog->is_this_news) ? 'checked' : '' }}>
                                    <label class="custom-control-label" for="switch_news"></label>
                                </div>
                                <div id="news_error" class="text-danger"> @error('point')
                                        {{ $message }}
                                    @enderror
                                </div>
                            </div>
                            ...
                            <div class="col-sm-9">
                                <div class="layout-button mt-25">
                                    ...
                                    <a class="btn btn-default btn-squared border-normal bg-normal px-30 "
                                        href="{{ route('admin.blogs.list') }}">{{ trans('label.cancel') }}</a>
                                </div>
                            </div>
                        </div>
                        {!! Form::close() !!}
...
form.submit();
 ...
 $(input).parent().parent().siblings('#' + input.name + '_error').html('')
    ...
    <script type="text/javascript">
        ...
        function changeBlogNews(_this) {
            var status = $(_this).prop('checked') == true ? 1 : 0;
            var $collectedIsVisible = '';
            if ($('#switch_news').is(':checked')) {
                $collectedIsVisible = $('#switch_news').attr('value', 1);
            } else {
                $collectedIsVisible = $('#switch_news').attr('value', 0);
            }
        }
    </script>
@endsection
