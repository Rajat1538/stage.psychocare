...
@section('title')
    {{ trans('label.deal_with_range_image') }}
@endsection
@section('page_title')
    {{ trans('label.deal_with_range_image') }}
@endsection
                ...
                <div class="card-body py-md-30">
                    <div class="card card-default card-md mb-4">
                        <ul class="atbd-breadcrumb nav">
                            ...
                            <li class="atbd-breadcrumb__item">
                                @if (isset($dealwithrangeimage) && !empty($dealwithrangeimage))
                                    Edit Deal With Range Image
                                @else
                                    Add Deal With Range Image
                                @endif
                            </li>
                        </ul>
                    </div>
                    <div class="horizontal-form">
                        <!-- Form -->
                        @if (!empty($dealwithrangeimage))
                            {!! Form::open([
                                'route' => 'admin.deal_with_range_image.update',
                                'method' => 'POST',
                                'files' => true,
                            ...
                            {!! Form::hidden('id', isset($dealwithrangeimage->id) ? $dealwithrangeimage->id : '', [
                                'class' => 'form-control',
                                'id' => 'hiddenId',
                            ]) !!}
                        @else
                            {{ Form::open(['route' => 'admin.deal_with_range_image.store', 'method' => 'POST', 'files' => true, 'class' => 'form-horizontal', 'id' => 'form', 'enctype' => 'multipart/form-data']) }}
                                ...
                                <div class="account-profile d-flex align-items-center">
                                    <div class="ap-img pro_img_wrapper">
                                        ...
                                        <label for="image">
                                            ...
                                            if (isset($dealwithrangeimage->image) && !empty($dealwithrangeimage->image)) {
                                                $mainImageUrl = asset('storage/app/public/uploads/dealwithrangeimage') . '/' . $dealwithrangeimage->image;
                                            }
                                            ?>
                                            <input type="hidden" name="old_image" id="old_image"
                                                value="{{ isset($dealwithrangeimage->image) && !empty($dealwithrangeimage->image) ? $dealwithrangeimage->image : '' }}">
                            ...
                            <div class="col-sm-9 d-flex align-items-center">
                                <div class="custom-control custom-switch switch-primary switch-md ">
                                    <input type="checkbox" name="status" class="custom-control-input" id="switch_status"
                                        ...
                                        {{ (isset($dealwithrangeimage->status) && $dealwithrangeimage->status == 1) || !isset($dealwithrangeimage->status) ? 'checked' : '' }}>
                            ...
                            <div class="col-sm-9">
                                <div class="layout-button mt-25">
                                    ...
                                    <a class="btn btn-default btn-squared border-normal bg-normal px-30 "
                                        href="{{ route('admin.deal_with_range_image.list') }}">{{ trans('label.cancel') }}</a>
                                </div>
                            </div>
                        </div>
                        {!! Form::close() !!}
