                ...
                <div class="card-body py-md-30">
                    <div class="card card-default card-md mb-4">
                        <ul class="atbd-breadcrumb nav">
                            ...
                            <li class="atbd-breadcrumb__item">
                                @if (isset($contractManufacturer))
                                    Edit Contract Manufacturer
                                @else
                                    Add Contract Manufacturer
                                @endif
                            </li>
                        </ul>
                    </div>
                    ...
                    <div class="horizontal-form">
                        ...
                        @if (!empty($contractManufacturer))
                            {!! Form::open([
                                'route' => 'admin.contractmanufacturer.update',
                                'method' => 'POST',
                                'files' => true,
                            ...
                            {!! Form::hidden('id', isset($contractManufacturer->id) ? $contractManufacturer->id : '', [
                                'class' => 'form-control',
                                'id' => 'hiddenId',
                            ]) !!}
                        @else
                            {{ Form::open(['route' => 'admin.contractmanufacturer.store', 'method' => 'POST', 'files' => true, 'class' => 'form-horizontal', 'id' => 'form', 'enctype' => 'multipart/form-data']) }}
                                ...
                                <div class="account-profile d-flex align-items-center">
                                    <div class="ap-img pro_img_wrapper">
                                        ...
                                        <label for="image">
                                            ...
                                            if (isset($contractManufacturer->image) && !empty($contractManufacturer->image)) {
                                                $mainImageUrl = asset('storage/app/public/uploads/contractmanufacturer/image') . '/' . $contractManufacturer->image;
                                            }
                                            ?>
                                            <input type="hidden" name="old_image" id="old_image"
                                                value="{{ isset($contractManufacturer->image) && !empty($contractManufacturer->image) ? $contractManufacturer->image : '' }}">
                            ...
                            <div class="col-sm-9 d-flex align-items-center">
                                <div class="custom-control custom-switch switch-primary switch-md ">
                                    <input type="checkbox" name="status" class="custom-control-input" id="switch_status"
                                        ...
                                        {{ (isset($contractManufacturer->status) && $contractManufacturer->status == 1) || !isset($contractManufacturer->status) ? 'checked' : '' }}>
                            ...
                            <div class="col-sm-9">
                                <div class="layout-button mt-25">
                                    <button type="submit" class="btn btn-sm btn-primary btn-add px-30"
                                        ...
                                        @if (isset($contractManufacturer) && !empty($contractManufacturer))
                                            {{ trans('label.update') }}
                                        @else
                                            {{ trans('label.save') }}
                                        @endif
                                    ...
                                    <a class="btn btn-default btn-squared border-normal bg-normal px-30 "
                                        href="{{ route('admin.contractmanufacturer.list') }}">{{ trans('label.cancel') }}</a>
                                </div>
                            </div>
                        </div>
                        {!! Form::close() !!}
