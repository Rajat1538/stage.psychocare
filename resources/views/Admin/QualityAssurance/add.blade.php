                ...
                <div class="card-body py-md-30">
                    ...
                    <div class="horizontal-form">
                        ...
                        @if (!empty($qualityAssurance))
                            {!! Form::open([
                                'route' => 'admin.qualityassurance.update',
                                'method' => 'POST',
                                'files' => true,
                            ...
                            {!! Form::hidden('id', isset($qualityAssurance->id) ? $qualityAssurance->id : '', [
                                'class' => 'form-control',
                                'id' => 'hiddenId',
                            ]) !!}
                        @else
                            {{ Form::open(['route' => 'admin.qualityassurance.store', 'method' => 'POST', 'files' => true, 'class' => 'form-horizontal', 'id' => 'form', 'enctype' => 'multipart/form-data']) }}
                                ...
                                <div class="account-profile d-flex align-items-center">
                                    <div class="ap-img pro_img_wrapper">
                                        ...
                                        <label for="image">
                                            ...
                                            if (isset($qualityAssurance->image) && !empty($qualityAssurance->image)) {
                                                $mainImageUrl = asset('storage/app/public/uploads/qualityassurance/image') . '/' . $qualityAssurance->image;
                                            }
                                            ?>
                                            <input type="hidden" name="old_image" id="old_image"
                                                value="{{ isset($qualityAssurance->image) && !empty($qualityAssurance->image) ? $qualityAssurance->image : '' }}">
                            ...
                            <div class="col-sm-9 d-flex align-items-center">
                                <div class="custom-control custom-switch switch-primary switch-md ">
                                    <input type="checkbox" name="status" class="custom-control-input" id="switch_status"
                                        ...
                                        {{ (isset($qualityAssurance->status) && $qualityAssurance->status == 1) || !isset($qualityAssurance->status) ? 'checked' : '' }}>
                            ...
                            <div class="col-sm-9">
                                <div class="layout-button mt-25">
                                    <button type="submit" class="btn btn-sm btn-primary btn-add px-30"
                                        ...
                                        @if (isset($qualityAssurance) && !empty($qualityAssurance))
                                            {{ trans('label.update') }}
                                        @else
                                            {{ trans('label.save') }}
                                        @endif
                                    ...
                                    <a class="btn btn-default btn-squared border-normal bg-normal px-30 "
                                        href="{{ route('admin.qualityassurance.list') }}">{{ trans('label.cancel') }}</a>
                                </div>
                            </div>
                        </div>
                        {!! Form::close() !!}
