                ...
                <div class="card-body py-md-30">
                    <div class="card card-default card-md mb-4">
                        <ul class="atbd-breadcrumb nav">
                            ...
                            <li class="atbd-breadcrumb__item">
                                @if (isset($logisticPartner))
                                    Edit Logistic Partner
                                @else
                    ...
                    <div class="horizontal-form">
                        ...
                        @if (!empty($logisticPartner))
                            {!! Form::open([
                                'route' => 'admin.logisticpartner.update',
                                'method' => 'POST',
                                'files' => true,
                            ...
                            {!! Form::hidden('id', isset($logisticPartner->id) ? $logisticPartner->id : '', [
                                'class' => 'form-control',
                                'id' => 'hiddenId',
                            ]) !!}
                        @else
                            {{ Form::open(['route' => 'admin.logisticpartner.store', 'method' => 'POST', 'files' => true, 'class' => 'form-horizontal', 'id' => 'form', 'enctype' => 'multipart/form-data']) }}
                                ...
                                <div class="account-profile d-flex align-items-center">
                                    <div class="ap-img pro_img_wrapper">
                                        ...
                                        <label for="image">
                                            ...
                                            if (isset($logisticPartner->image) && !empty($logisticPartner->image)) {
                                                $mainImageUrl = asset('storage/app/public/uploads/logisticpartner/image') . '/' . $logisticPartner->image;
                                            }
                                            ?>
                                            <input type="hidden" name="old_image" id="old_image"
                                                value="{{ isset($logisticPartner->image) && !empty($logisticPartner->image) ? $logisticPartner->image : '' }}">
                            ...
                            <div class="col-sm-9 d-flex align-items-center">
                                <div class="custom-control custom-switch switch-primary switch-md ">
                                    <input type="checkbox" name="status" class="custom-control-input" id="switch_status"
                                        ...
                                        {{ (isset($logisticPartner->status) && $logisticPartner->status == 1) || !isset($logisticPartner->status) ? 'checked' : '' }}>
                            ...
                            <div class="col-sm-9">
                                <div class="layout-button mt-25">
                                    <button type="submit" class="btn btn-sm btn-primary btn-add px-30"
                                        ...
                                        @if (isset($logisticPartner) && !empty($logisticPartner))
                                            {{ trans('label.update') }}
                                        @else
                                            {{ trans('label.save') }}
                                        @endif
                                    ...
                                    <a class="btn btn-default btn-squared border-normal bg-normal px-30 "
                                        href="{{ route('admin.logisticpartner.list') }}">{{ trans('label.cancel') }}</a>
                                </div>
                            </div>
                        </div>
                        {!! Form::close() !!}
